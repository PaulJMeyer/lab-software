...
@pytest.fixture
def samples_dict():
    sample_a = Sample(sample_id="111111111", sample_dna="ACGT")
    sample_b = Sample(sample_id="222222222", sample_dna="TTTT")
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_roundtrip_preserves_derived_analysis_fields(self, tmp_path):
        path = tmp_path / "lab_state.json"
        sample = Sample(
            sample_id="111111111",
            sample_dna="ACGT",
        ...
        save_samples(path, {sample.sample_id: sample})
        loaded = load_samples(path)

        assert loaded["111111111"].reverse_complement == "ACGT"
        assert loaded["111111111"].rna_transcript == "ACGU"
        assert loaded["111111111"].protein == "M"

    def test_roundtrip_with_no_derived_fields_stays_none(self, tmp_path, samples_dict):
        path = tmp_path / "lab_state.json"
        save_samples(path, samples_dict)
        loaded = load_samples(path)
        assert loaded["111111111"].reverse_complement is None
        assert loaded["111111111"].rna_transcript is None
        assert loaded["111111111"].protein is None
